...
def process_record(record):
    ...
    for label_detection in response["Labels"]:
        ...
        data = {"base_timestamp": base_ts, "ts": timestamp, "name": name, "confidence": confidence,
            "destinationBucket": destination_bucket, "videoClipName":video_clip_name
            }
        logger.info("Saving all identified labels from current job to opensearch index")
        response = requests.post(f"https://{es_url}/entities/_doc/",
                                     auth=aws_auth,
                                     headers={"Content-Type": "application/json"},
                                     json=data,
                                     timeout=60)
        logger.info(f"Result after saving to opensearch: code={response.status_code}, response={response.text}")
    else:
